...
CREATE TABLE IF NOT EXISTS customers (
    ...
    cpf VARCHAR(11) NOT NULL UNIQUE,  -- Corrigido: CPF tem 11 dígitos
...
COMMENT ON COLUMN customers.cpf IS 'CPF number - 11 digits';  -- Corrigido: comentário
...
-- Add comments to simulations table
COMMENT ON TABLE simulations IS 'Loan simulation requests and calculations';
COMMENT ON COLUMN simulations.requested_amount IS 'Amount requested by customer in USD';
COMMENT ON COLUMN simulations.monthly_interest_rate IS 'Monthly interest rate as percentage';
COMMENT ON COLUMN simulations.status IS 'Current status of the simulation';
...
CREATE TABLE IF NOT EXISTS installments (
    ...
    CONSTRAINT fk_installment_simulation FOREIGN KEY (simulation_id) REFERENCES simulations(id) ON DELETE CASCADE,
    ...
    CONSTRAINT uk_simulation_installment UNIQUE (simulation_id, installment_number)
);
...
CREATE INDEX IF NOT EXISTS idx_simulation_code ON simulations(code);
